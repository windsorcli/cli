...
func TestInitCmd(t *testing.T) {
	...
	t.Run("RunEPlatformAndProviderConflict", func(t *testing.T) {
		...
		// Then an error should occur due to conflict
		if err == nil {
			t.Error("Expected error for conflicting platform and provider flags, got nil")
		}
		if !strings.Contains(err.Error(), "cannot specify both --provider and --platform flags") {
