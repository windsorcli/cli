...
func TestDefaultWorkstationRuntime(t *testing.T) {
	t.Run("ReturnsDockerDesktopOrDockerDependingOnHostOS", func(t *testing.T) {
		got := defaultWorkstationRuntime()
		switch got {
		case "docker-desktop", "docker":
			// Accept either; precise value depends on GOOS at test time.
		default:
			t.Errorf("Expected docker-desktop or docker, got %q", got)
		}
	})
}

func TestApplyLocalWorkstationDefault(t *testing.T) {
	setup := func(t *testing.T, contextName, persistedRuntime string) *runtime.Runtime {
		t.Helper()
	...
	t.Run("AppliesDefaultWhenLocalContextAndNoRuntimeSet", func(t *testing.T) {
		// Given a local context with no persisted workstation.runtime and no flag override
		rt := setup(t, "local", "")
		overrides := map[string]any{}

		// When the helper is applied
		applyLocalWorkstationDefault(rt, overrides)

		// Then workstation.runtime is set to an OS-appropriate default and platform is set to docker
		if overrides["workstation.runtime"] == nil {
			t.Fatal("Expected workstation.runtime to be set, got nil")
		}
		switch overrides["workstation.runtime"] {
		case "docker-desktop", "docker":
			// OK
		default:
			t.Errorf("Expected workstation.runtime to be docker-desktop or docker, got %v", overrides["workstation.runtime"])
		}
		if overrides["platform"] != "docker" {
			t.Errorf("Expected platform=docker, got %v", overrides["platform"])
		}
	})

	t.Run("NoOpWhenContextIsNotLocal", func(t *testing.T) {
		// Given a non-local context
		rt := setup(t, "aws", "")
		overrides := map[string]any{}
		...
		if len(overrides) != 0 {
			t.Errorf("Expected empty overrides for non-local context, got %v", overrides)
		}
	})

	t.Run("NoOpWhenWorkstationRuntimeAlreadyPersisted", func(t *testing.T) {
		// Given a local context with workstation.runtime already persisted in config
		rt := setup(t, "local", "colima")
		overrides := map[string]any{}
		...
		// Then no overrides are injected
		if len(overrides) != 0 {
			t.Errorf("Expected empty overrides when runtime already persisted, got %v", overrides)
		}
	})

	t.Run("NoOpWhenOverrideAlreadySetFromFlag", func(t *testing.T) {
		// Given a local context with a flag-provided workstation.runtime
		rt := setup(t, "local", "")
		overrides := map[string]any{"workstation.runtime": "colima"}

		// When the helper is applied
		applyLocalWorkstationDefault(rt, overrides)

		// Then the existing override is preserved
		if overrides["workstation.runtime"] != "colima" {
			t.Errorf("Expected flag-provided runtime=colima to be preserved, got %v", overrides["workstation.runtime"])
		}
		if _, hasPlatform := overrides["platform"]; hasPlatform {
			t.Errorf("Expected platform to not be added when runtime came from flag, got %v", overrides["platform"])
		}
	})

	t.Run("PreservesExplicitPlatformWhenInjectingRuntime", func(t *testing.T) {
		// Given a local context with an explicit platform override but no runtime
		rt := setup(t, "local", "")
		...
		// Then the runtime default is injected but the explicit platform is kept
		if overrides["workstation.runtime"] == nil {
			t.Error("Expected workstation.runtime to be set")
		}
		if overrides["platform"] != "metal" {
			t.Errorf("Expected explicit platform=metal to be preserved, got %v", overrides["platform"])
		}
	})
}
