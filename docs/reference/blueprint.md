...
| `crds` | `array<object>` | The vendored CRD layer: kustomizations under the crds/ path that the provisioner applies as their own phase ahead of the kustomize: layer, so every kustomization sees its CRDs Established first. Populated by the composer from facet crds: declarations; each entry has the same shape as a kustomize: entry (name, path, prune, wait, ...). Internally CRDs are part of the kustomization set; this section is the presentation split. |
