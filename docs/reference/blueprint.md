...
| `backend` | `string` | Names the terraform component that terminates the backend's component chain. When set, 'windsor bootstrap' applies the backend component (and every component declared before it) against local state first, then migrates state to the configured remote backend before applying the rest of the graph. |
