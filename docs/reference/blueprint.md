...
| `crds` | `array<string>` | The vendored CRD layer: a list of references into the crds/ catalog (e.g. 'cert-manager-1.16.2'). The provisioner materializes these into a single 'crds' kustomization (its components are these references, pruning disabled and wait enabled) applied ahead of the kustomize: layer, so every kustomization sees its CRDs Established first. Populated by the composer from facet crds: declarations; the stack depends on the layer by the name 'crds'. |
