...
| `id` | `string` | Stable identifier for the context, distinct from its map key. Used for cross-context references where the key may change. Starts with a lowercase letter. Contains only lowercase letters and digits. |
