...
| `id` | `string` | Stable identifier for the context, distinct from its map key. Used for cross-context references where the key may change. Starts with a lowercase letter. Contains lowercase letters, digits, and single hyphens. Does not start, end, or repeat a hyphen. |
