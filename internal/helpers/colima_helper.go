...
func (h *ColimaHelper) WriteConfig() error {
	...
	tempFilePath := filepath.Join(os.Getenv("HOME"), fmt.Sprintf(".colima/windsor-%s", context), "colima.yaml") + ".tmp"
	...
	if err := rename(tempFilePath, filepath.Join(os.Getenv("HOME"), fmt.Sprintf(".colima/windsor-%s", context), "colima.yaml")); err != nil {
