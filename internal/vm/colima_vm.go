...
// NewColimaVM creates a new instance of ColimaVM
func NewColimaVM(configHandler config.ConfigHandler, context context.ContextInterface, shell shell.Shell) *ColimaVM {
	return &ColimaVM{
...
func (vm *ColimaVM) Up(verbose ...bool) error {
	...
	contextConfig, err := vm.ConfigHandler.GetConfig()
	if err != nil {
		return fmt.Errorf("error retrieving config: %w", err)
	}

	contextName, err := vm.Context.GetContext()
	if err != nil {
	...
	if contextConfig != nil && contextConfig.VM != nil && contextConfig.VM.Driver != nil {
		...
		if *contextConfig.VM.Driver == "colima" {
			...
			output, err := vm.Shell.Exec(verbose[0], "Executing colima start command", command, args...)
			if err != nil {
...
func (vm *ColimaVM) Down(verbose ...bool) error {
	...
	command := "colima"
	args := []string{"stop", fmt.Sprintf("windsor-%s", contextName)}
	output, err := vm.Shell.Exec(verbose[0], "Executing colima stop command", command, args...)
	if err != nil {
		return fmt.Errorf("Error executing command %s %v: %w\n%s", command, args, err, output)
	}
...
func (vm *ColimaVM) Delete(verbose ...bool) error {
	...
	command := "colima"
	args := []string{"delete", fmt.Sprintf("windsor-%s", contextName)}
	output, err := vm.Shell.Exec(verbose[0], "Executing colima delete command", command, args...)
	if err != nil {
		return fmt.Errorf("Error executing command %s %v: %w\n%s", command, args, err, output)
	}
...
func (vm *ColimaVM) Info() (interface{}, error) {
	...
	out, err := vm.Shell.Exec(false, "Fetching Colima info", command, args...)
	if err != nil {
		return nil, err
	}
...
func (vm *ColimaVM) writeConfig() error {
	context, err := vm.Context.GetContext()
	if err != nil {
		return fmt.Errorf("error retrieving context: %w", err)
	}

	config, err := vm.ConfigHandler.GetConfig()
	if err != nil {
		return fmt.Errorf("error retrieving config: %w", err)
	}
