...
func (a *ArtifactBuilder) Create(outputPath string, tag string) (string, error) {
	...
	if finalName == "" {
		return "", fmt.Errorf("name is required: provide via --tag name:version or metadata.yaml")
	}
	if finalVersion == "" {
		return "", fmt.Errorf("version is required: provide via --tag name:version or metadata.yaml")
	}
...
// resolveOutputPath determines the final output path, handling directory vs file paths.
// If outputPath is default "blueprint.tar.gz", generates filename from name-version.
// If outputPath is a directory (existing or ends with separator), places generated filename in directory.
// Otherwise treats outputPath as explicit filename and uses as-is.
func (a *ArtifactBuilder) resolveOutputPath(outputPath, name, version string) string {
	// Generate suggested filename
	suggestedFilename := fmt.Sprintf("%s-%s.tar.gz", name, version)

	// Default case: replace default with generated filename
	if outputPath == "blueprint.tar.gz" {
		return suggestedFilename
	}

	// Check if output path is a directory
	if stat, err := a.shims.Stat(outputPath); err == nil && stat.IsDir() {
		return filepath.Join(outputPath, suggestedFilename)
	}

	// Check if path ends with separator (intended as directory)
	if strings.HasSuffix(outputPath, "/") || strings.HasSuffix(outputPath, "\\") {
		return filepath.Join(outputPath, suggestedFilename)
	}

	// Otherwise treat as explicit filename
	return outputPath
}

// generateMetadataWithNameVersion creates complete metadata for bundled artifacts using provided name and version.
// It takes a BlueprintMetadataInput (potentially empty), name and version strings, then extracts git provenance
// and builder information to construct a complete BlueprintMetadata structure with timestamp and marshals it to YAML.
// Git provenance and builder info are best-effort and will be empty if unavailable.
