...
func TestTalosEnv_Print(t *testing.T) {
	...
	t.Run("WithCustomVars", func(t *testing.T) {
		...
		customVars := map[string]string{
			"CUSTOM_TALOS_VAR1": "custom-value1",
			"CUSTOM_TALOS_VAR2": "custom-value2",
		}
		...
		err := talosEnvPrinter.Print(customVars)
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
		...
		if _, exists := capturedEnvVars["TALOSCONFIG"]; !exists {
			t.Errorf("expected TALOSCONFIG to exist in capturedEnvVars")
		}
	})
