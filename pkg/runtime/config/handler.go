...
// contextIDCharset is the character set a context id is built from, whether generated by
// GenerateContextID or validated by validateContextID.
...
// validateContextID checks id against GenerateContextID's format: a lowercase letter, then only
// contextIDCharset characters. An empty id is valid; GenerateContextID fills it in later.
func validateContextID(id string) error {
	...
	if strings.Trim(id, contextIDCharset) != "" {
		return fmt.Errorf("id %q must contain only lowercase letters and digits", id)
	}
	return nil
}
