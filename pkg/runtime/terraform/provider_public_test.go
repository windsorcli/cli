...
func TestTerraformProvider_GenerateLocalBackendOverride(t *testing.T) {
	t.Run("WritesLocalBackendStanza", func(t *testing.T) {
		// Given a provider (backend type in config is irrelevant for the local override)
		mocks := setupMocks(t, &SetupOptions{BackendType: "local"})
		...
		// When generating the local backend override
		err := mocks.Provider.GenerateLocalBackendOverride("/test/dir")

		// Then the local backend stanza is written to backend_override.tf
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		...
		expectedPath := filepath.Join("/test/dir", "backend_override.tf")
		if writtenPath != expectedPath {
			t.Errorf("Expected write path %s, got %s", expectedPath, writtenPath)
		}

		expectedConfig := "terraform {\n  backend \"local\" {}\n}"
		if string(writtenData) != expectedConfig {
			t.Errorf("Expected backend config %q, got %q", expectedConfig, string(writtenData))
		}
	})

	t.Run("OverwritesExistingOverride", func(t *testing.T) {
		// Given a provider and a directory that already contains a backend_override.tf
		mocks := setupMocks(t, &SetupOptions{BackendType: "s3"})
		...
		// Then the file is written unconditionally (no Stat probe first)
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		if statCalled {
			t.Error("Expected GenerateLocalBackendOverride not to probe the filesystem before writing")
		}
		if !writeCalled {
	...
	t.Run("IgnoresConfiguredBackendType", func(t *testing.T) {
		// Given a provider whose configured backend type is s3
		mocks := setupMocks(t, &SetupOptions{BackendType: "s3"})
		...
		// Then the written content is the local stanza regardless of the configured type
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		...
		expectedConfig := "terraform {\n  backend \"local\" {}\n}"
		if string(writtenData) != expectedConfig {
			t.Errorf("Expected local stanza, got %q", string(writtenData))
		}
	})

	t.Run("HandlesWriteFileError", func(t *testing.T) {
		// Given a provider whose WriteFile shim fails
		mocks := setupMocks(t, &SetupOptions{BackendType: "local"})
		...
		if !strings.Contains(err.Error(), "backend_override.tf") {
			t.Errorf("Expected error to reference backend_override.tf, got %v", err)
		}
...
func TestTerraformProvider_GenerateTerraformArgsForcedLocal(t *testing.T) {
	t.Run("EmitsLocalBackendConfigArgs", func(t *testing.T) {
		// Given a provider whose configured backend type is s3
		mocks := setupMocks(t, &SetupOptions{BackendType: "s3"})
		...
		// When generating forced-local terraform args
		args, err := mocks.Provider.GenerateTerraformArgsForcedLocal("test/path", true)

		// Then InitArgs include a local-path backend-config and no s3 key arg
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}

		var foundPath bool
		for _, a := range args.InitArgs {
			if strings.HasPrefix(a, "-backend-config=path=") {
				foundPath = true
			}
			if strings.HasPrefix(a, "-backend-config=key=") {
				t.Errorf("Expected no s3 key backend-config arg when forced local, got %q", a)
			}
		}
		if !foundPath {
			t.Errorf("Expected a -backend-config=path=... arg in InitArgs, got %v", args.InitArgs)
		}
	})

	t.Run("MatchesGenerateTerraformArgsForNonBackendFields", func(t *testing.T) {
		// Given a provider with any backend configured
		mocks := setupMocks(t, &SetupOptions{BackendType: "local"})
		...
		// When generating args both ways
		normal, err := mocks.Provider.GenerateTerraformArgs("test/path", true)
		if err != nil {
			t.Fatalf("GenerateTerraformArgs failed: %v", err)
		}
		forced, err := mocks.Provider.GenerateTerraformArgsForcedLocal("test/path", true)
		if err != nil {
			t.Fatalf("GenerateTerraformArgsForcedLocal failed: %v", err)
		}

		// Then non-backend fields are identical between the two
		if normal.TFDataDir != forced.TFDataDir {
			t.Errorf("TFDataDir differs: normal=%q forced=%q", normal.TFDataDir, forced.TFDataDir)
		}
		if strings.Join(normal.PlanArgs, " ") != strings.Join(forced.PlanArgs, " ") {
			t.Errorf("PlanArgs differ: normal=%v forced=%v", normal.PlanArgs, forced.PlanArgs)
		}
		if strings.Join(normal.ApplyArgs, " ") != strings.Join(forced.ApplyArgs, " ") {
			t.Errorf("ApplyArgs differ: normal=%v forced=%v", normal.ApplyArgs, forced.ApplyArgs)
		}
		if strings.Join(normal.RefreshArgs, " ") != strings.Join(forced.RefreshArgs, " ") {
			t.Errorf("RefreshArgs differ: normal=%v forced=%v", normal.RefreshArgs, forced.RefreshArgs)
		}
		if strings.Join(normal.DestroyArgs, " ") != strings.Join(forced.DestroyArgs, " ") {
			t.Errorf("DestroyArgs differ: normal=%v forced=%v", normal.DestroyArgs, forced.DestroyArgs)
		}
	})

	t.Run("PropagatesScratchPathError", func(t *testing.T) {
		// Given a provider whose scratch path lookup fails
		mocks := setupMocks(t, &SetupOptions{BackendType: "s3"})

		mocks.ConfigHandler.GetWindsorScratchPathFunc = func() (string, error) {
			return "", errors.New("scratch boom")
		}

		// When generating forced-local terraform args
		_, err := mocks.Provider.GenerateTerraformArgsForcedLocal("test/path", true)

		// Then the error is surfaced
		if err == nil {
			t.Fatal("Expected error from scratch path failure")
		}
		if !strings.Contains(err.Error(), "scratch boom") {
			t.Errorf("Expected error to wrap scratch path failure, got %v", err)
		}
	})
}
