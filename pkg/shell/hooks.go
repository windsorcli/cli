...
var shellHooks = map[string]string{
	...
	"fish": `
		function _windsor_hook --on-event fish_prompt
			eval (eval "$("{{.SelfPath}}" env --decrypt)"; export fish | source)
		end
		`,
	"tcsh": `
		alias precmd '_windsor_hook';
		alias _windsor_hook 'eval eval "$("{{.SelfPath}}" env --decrypt)"; export tcsh"
		`,
	"elvish": `
		eval (eval "$("{{.SelfPath}}" env --decrypt)"; export elvish | slurp)
		`,
	"powershell": `
		...
		function prompt {
				$windsorEnvScript = & "{{.SelfPath}}" env --decrypt | Out-String
				if ($windsorEnvScript) {
