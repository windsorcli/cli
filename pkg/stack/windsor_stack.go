...
func (s *WindsorStack) Up() error {
	...
	for _, component := range components {
		...
		_, err = s.shell.ExecProgress(fmt.Sprintf("🌎 Initializing Terraform in %s", component.FullPath), "terraform", "init", "-migrate-state", "-upgrade")
		if err != nil {
			return fmt.Errorf("error initializing Terraform in %s: %w", component.FullPath, err)
		}

		// Execute 'terraform plan' in the dirPath
		_, err = s.shell.ExecProgress(fmt.Sprintf("🌎 Planning Terraform changes in %s", component.FullPath), "terraform", "plan")
		if err != nil {
			return fmt.Errorf("error planning Terraform changes in %s: %w", component.FullPath, err)
		}

		// Execute 'terraform apply' in the dirPath
		_, err = s.shell.ExecProgress(fmt.Sprintf("🌎 Applying Terraform changes in %s", component.FullPath), "terraform", "apply")
		if err != nil {
